...
def test_cache_v2_is_rejected_and_refreshed(tmp_path):
    ...
    cache._path("estimate", "ABC").write_text(
        json.dumps(
            {
    ...
    item, status = cache.get_or_fetch(
...
def test_setup_none_is_not_ranked():
    result = setup_scores(
        {
            "drawdown_from_20d_high_pct": -30,
            "drawdown_from_60d_high_pct": -15,
            "return_1d_pct": -8,
            "return_5d_pct": -5,
            "return_20d_pct": -10,
            "return_60d_pct": -20,
            "price_vs_ma20_pct": -10,
            "price_vs_ma50_pct": -20,
            "price_vs_ma200_pct": -30,
            "relative_volume_1d": 0.5,
            "relative_volume_5d": 2,
            "rs_60d_percentile": 0,
        }
    )
...
def test_target_upside_is_independent_of_range_and_mean_fallback():
    frame = pd.DataFrame(
        {
            "target_median": [190, 190, np.nan],
            "target_mean": [180, 180, 180],
            "target_low": [np.nan, 210, np.nan],
            "target_high": [np.nan, 200, np.nan],
            "current_price": [150, 150, 150],
        }
    )
    result = target_metrics(frame)
    assert result.target_upside_pct.tolist() == pytest.approx(
        [26.6666667, 26.6666667, 20]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "price_coverage_pct": 98,
                "quality_coverage_pct": 80,
                "valuation_coverage_pct": 80,
                "expectations_coverage_pct": 80,
            },
            "VALID",
        ),
        (
            {
                ...
                "quality_coverage_pct": 60,
                "valuation_coverage_pct": 60,
                "expectations_coverage_pct": 65,
            },
            "DEGRADED",
        ),
        (
            {
                "price_coverage_pct": 89,
                "quality_coverage_pct": 80,
                "valuation_coverage_pct": 80,
                "expectations_coverage_pct": 80,
            },
...
def test_manifest_has_versions_coverage_and_config(tmp_path, monkeypatch):
    ...
    assert manifest["application_version"] == "1.1.3"
    ...
    assert (
        json.loads((context.directory / "run_manifest.json").read_text())["config"][
            "top_n"
        ]
        == 10
    )
...
def test_forward_return_uses_trading_sessions_and_maturity_guards():
    ...
    assert [sample_status(n) for n in (19, 20, 60)] == [
        "INSUFFICIENT_SAMPLE",
        "EARLY_SAMPLE",
        "USABLE",
    ]
...
def test_resume_skips_success_and_retries_failure(tmp_path, monkeypatch):
    ...
    def fetch(_client, pending):
        ...
        return pd.DataFrame(
            {"symbol": pending.symbol, "provider_success": [1] * len(pending)}
        )
...
def test_replay_uses_stored_inputs_without_yahoo(tmp_path, monkeypatch):
    ...
    universe = pd.DataFrame(
        {"symbol": ["ABC"], "index_name": ["S&P 500"], "sector": ["Tech"]}
    )
    ...
    full = universe.assign(
        ...
        growth_score=80,
        valuation_score=80,
        expectations_long_score=80,
        risk_score=20,
        confidence_score=90,
        short_term_setup="PULLBACK",
        benchmark_rs_status="FALLBACK_RAW",
    )
    ...
    monkeypatch.setattr(
        app.YahooClient, "fetch_many", lambda *_: pytest.fail("Yahoo called in replay")
    )
