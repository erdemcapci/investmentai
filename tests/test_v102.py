...
from investment_ai.runtime import RunContext, atomic_json, build_manifest, scoring_code_fingerprint
...
def test_atomic_json_recursively_sanitizes_scientific_values(tmp_path):
    ...
    atomic_json(path, {
        "a": np.nan, "b": np.inf, "c": -np.inf, "d": np.float64(1.5),
        "e": np.int64(3), "f": pd.NA, "g": pd.NaT,
        "nested": {"x": [1, np.nan, pd.Timestamp("2026-01-02T03:04:05Z")]},
    })
    value = json.loads(path.read_text())
    assert value == {
        "a": None, "b": None, "c": None, "d": 1.5, "e": 3, "f": None,
        "g": None, "nested": {"x": [1, None, "2026-01-02T03:04:05+00:00"]},
    }
...
def test_manifest_and_validation_json_accept_nan_metrics(tmp_path):
    ...
    manifest = build_manifest(context, {
        "risk_median": np.nan, "confidence_median": np.nan,
...
def _frame():
    return pd.DataFrame([{
        "symbol": "ABC", "long_term_score": 80.0, "long_term_rank": 1,
        "short_term_score": 70.0, "short_term_rank": 2, "short_term_setup": "PULLBACK",
    }])


@pytest.mark.parametrize("lt,st,expected", [
...
def test_health_gated_history(lt, st, expected, tmp_path):
    ...
    if rows:
        ...
        store.save_predictions("run", "2026-01-01T00:00:00+00:00", "3.1.1", rows,
                               lt, st, "DEGRADED")
    count = store.db.execute("SELECT count(*) FROM prediction_snapshots").fetchone()[0]
    if expected is None:
        assert count == 0
        assert store.db.execute("SELECT count(*) FROM ranking_history").fetchone()[0] == 0
    else:
...
def test_validation_filters_invalid_but_keeps_degraded(tmp_path):
    store = HistoryStore(tmp_path / "history.db")
    for run, lt, st in (("bad-lt", "INVALID", "VALID"),
                        ("bad-st", "VALID", "INVALID"),
                        ("degraded", "DEGRADED", "DEGRADED")):
        store.save_predictions(run, "2025-01-01T00:00:00+00:00", "3.1.1",
                               _frame().to_dict("records"), lt, st, "DEGRADED")
        store.db.execute(
...
def _source(tmp_path):
    ...
    source.mkdir()
    names = ("universe.csv", "price_features.csv", "normalized_provider.csv", "full_analysis.csv")
    for name in names:
        (source / name).write_text("symbol\nABC\n")
    manifest = {
        "scoring_model_version": "3.1.1",
        "scoring_code_fingerprint": scoring_code_fingerprint(),
        "artifact_sha256": {name: hashlib.sha256((source / name).read_bytes()).hexdigest()
                            for name in names},
    }
...
def test_v3_to_v4_migration_and_explicit_insert_columns(tmp_path):
    ...
    columns = {row[1] for row in store.db.execute("PRAGMA table_info(prediction_snapshots)")}
    assert {"lt_run_status", "st_run_status", "overall_run_status"} <= columns
    assert store.db.execute("SELECT symbol FROM prediction_snapshots").fetchone()[0] == "ABC"
    store.close()
...
def test_future_database_is_rejected(tmp_path):
    ...
    db.execute("INSERT INTO metadata VALUES ('database_schema_version','5')")
    db.commit()
    db.close()
...
def test_history_store_closes_after_fatal_execute_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(app, "build_analysis", lambda *_: (_ for _ in ()).throw(RuntimeError("boom")))
