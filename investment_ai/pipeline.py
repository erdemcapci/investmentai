...
def target_metrics(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    low = pd.to_numeric(
        frame.get("target_low", pd.Series(np.nan, index=frame.index)), errors="coerce"
    )
    high = pd.to_numeric(
        frame.get("target_high", pd.Series(np.nan, index=frame.index)), errors="coerce"
    )
    price = pd.to_numeric(
        frame.get("current_price", pd.Series(np.nan, index=frame.index)),
        errors="coerce",
    )
    ...
    return pd.DataFrame(
        {
            ...
            "target_upside_pct": np.where(
                selected.gt(0) & price.gt(0), (selected / price - 1) * 100, np.nan
            ),
            "target_dispersion_pct": np.where(
                valid_range & selected.gt(0),
                (high - low) / selected.abs() * 100,
                np.nan,
            ),
        },
        index=frame.index,
...
def _history_scores_v31(row):
    ...
    def revenue_curve(period, days):
        return curve(
            row.get(f"revenue_{period}_change_{days}d_pct"),
            [(-20, 0), (0, 50), (20, 100)],
        )

    q0 = weighted(
        {"7": revenue_curve("0q", 7), "30": revenue_curve("0q", 30)},
        {"7": 0.60, "30": 0.40},
        0,
    )[0]
    q1 = weighted(
        {"7": revenue_curve("plus_1q", 7), "30": revenue_curve("plus_1q", 30)},
        {"7": 0.50, "30": 0.50},
        0,
    )[0]
    revenue_short = weighted(
        {"0q": q0, "plus_1q": q1}, {"0q": 0.55, "plus_1q": 0.45}, 0
    )[0]
    y0 = weighted(
        {"30": revenue_curve("0y", 30), "90": revenue_curve("0y", 90)},
        {"30": 0.7, "90": 0.3},
        0,
    )[0]
    y1 = weighted(
        {"30": revenue_curve("plus_1y", 30), "90": revenue_curve("plus_1y", 90)},
        {"30": 0.7, "90": 0.3},
        0,
    )[0]
    revenue_long = weighted(
        {"0y": y0, "plus_1y": y1}, {"0y": 0.40, "plus_1y": 0.60}, 0
    )[0]
...
LT_DRIVER_SPECS = (
    ...
    ("Valuation vs peers", "Valuation", "valuation_score", 0.20),
    ("Long expectations", "Long Expectations", "expectations_long_score", 0.20),
    ("Long trend", "Long Trend", "long_trend_score", 0.10),
...
def _drivers(row, specs=LT_DRIVER_SPECS):
    ...
    positive = [
        f"+{d['contribution_points']:.1f} pts — {d['driver_name']}"
        for d in sorted(
            candidates, key=lambda x: x["contribution_points"], reverse=True
        )
        if d["contribution_points"] > 0
    ][:3]
    negative = [
        f"{d['contribution_points']:.1f} pts — {d['driver_name']}"
        for d in sorted(candidates, key=lambda x: x["contribution_points"])
        if d["contribution_points"] < 0
    ][:3]
    return (
...
def build_analysis(
    ...
    provider_price = pd.to_numeric(
        frame.get("current_price_provider", pd.Series(np.nan, index=frame.index)),
        errors="coerce",
    )
    completed_price = pd.to_numeric(
        frame.get("current_price", pd.Series(np.nan, index=frame.index)),
        errors="coerce",
    )
    ...
    frame[
        [
            "target_momentum_score",
            "revenue_revision_short_score",
            "revenue_revision_long_score",
        ]
    ] = history_scores
    ...
    frame["market_price_risk_score"] = [
        market_price_risk_score(row) for row in frame.to_dict("records")
    ]
    frame = pd.concat(
        [
            frame,
    ...
    frame[["lt_positive_drivers", "lt_negative_drivers", "lt_driver_contributions"]] = (
        lt_drivers
    )
    st_drivers = frame.apply(
        lambda row: _drivers(row, ST_DRIVER_SPECS), axis=1, result_type="expand"
    )
    frame[["st_positive_drivers", "st_negative_drivers", "st_driver_contributions"]] = (
        st_drivers
    )
