...
def weighted(components: dict[str,Any], weights: dict[str,float], minimum: float=0) -> tuple[float,float,str]:
    total = sum(weights.values()); available = [(number(components.get(k)),w) for k,w in weights.items() if pd.notna(number(components.get(k)))]
    coverage = sum(w for _,w in available)/total if total else 0
    if not available or coverage < minimum: return np.nan, coverage, INSUFFICIENT_DATA
    score = np.clip(sum(v*w for v,w in available)/sum(w for _,w in available), 0, 100)
